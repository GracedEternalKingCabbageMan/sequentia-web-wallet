...
// --- classify: on-chain cross offers ---
const ocAsk = classifyRelayOffer({ offer_id:'c1', base_amount:1000, want_amount:52, cross_chain:{ direction:0 } });
eq({ side:ocAsk.side, rail:ocAsk.rail, price:ocAsk.price }, { side:'ask', rail:'onchain', price:0.052 }, 'on-chain dir0 -> ask');
const ocBid = classifyRelayOffer({ offer_id:'c2', base_amount:1000, want_amount:58, cross_chain:{ direction:1 } });
eq({ side:ocBid.side, rail:ocBid.rail, price:ocBid.price }, { side:'bid', rail:'onchain', price:0.058 }, 'on-chain dir1 -> bid');
...
const raws = [
  { offer_id:'oc-ask', base_amount:1000, want_amount:52, cross_chain:{ direction:0 } },  // ask 0.052 onchain
  { offer_id:'ln-ask', offer_amount:1000, want_amount:50, lightning:{ ln_direction:4 } }, // ask 0.050 ln (cheaper)
  { offer_id:'oc-bid', base_amount:1000, want_amount:58, cross_chain:{ direction:1 } },   // bid 0.058 onchain (higher)
