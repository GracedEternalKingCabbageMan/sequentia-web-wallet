...
impl Decodable for BlockHeader {
    fn consensus_decode<D: io::Read>(mut d: D) -> Result<Self, encode::Error> {
        ...
        Ok(BlockHeader {
            ...
            // SEQUENTIA: struct-literal fields evaluate in written order, so the
            // anchor is read from the stream after `height` and before `ext`.
            #[cfg(feature = "sequentia")]
            bitcoin_anchor: {
...
mod tests {
    ...
    /// SEQUENTIA: a real Sequentia testnet block header (height 1352) fetched
    /// from the explorer esplora API. Proves the `sequentia` BlockHeader
    /// (de)serialization and the anchor-in-hash commitment match the chain.
    #[cfg(feature = "sequentia")]
    #[test]
    fn sequentia_testnet_header_roundtrip_and_hash() {
        use std::str::FromStr;
        let hex = include_str!("../test_data/sequentia_testnet_header.hex");
        let bytes = Vec::<u8>::from_hex(hex.trim()).unwrap();
        let header: BlockHeader = crate::encode::deserialize(&bytes).unwrap();

        assert_eq!(header.height, 1352);
        assert_eq!(
            header.bitcoin_anchor,
            Some((
                141030,
                BlockHash::from_str(
                    "00000000001879871fda237a76fef797f37eda34f46dc4dfdcd73a90226c40f9"
                )
                .unwrap()
            ))
        );
        // Re-hashing reproduces the chain's block hash: the anchor is committed
        // in the hash exactly as Sequentia's src/primitives/block.h does.
        assert_eq!(
            header.block_hash().to_string(),
            "a509dba2273a7393201ad7866f48ea5a869eff657c5416644797b88e3c61d9df"
        );
        // And the header re-serializes byte-for-byte.
        assert_eq!(crate::encode::serialize(&header), bytes);
    }
